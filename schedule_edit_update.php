...
while($rowschedule = mysqli_fetch_assoc($resultschedule)) {
	...
	if($song=="*"){
		...
		if($random==1){$script = "/usr/bin/mpg123 -q /var/www/html/music/".$_POST["song"].".mp3";}
		if($random==0){$script = "/usr/bin/mpg123 -q /var/www/html/music/no-random/".$_POST["song"].".mp3";}	
	}	
	...
	if($song=="*"){
		...
		$script = "/usr/bin/mpg123 -z /var/www/html/music/".$song.".mp3";
